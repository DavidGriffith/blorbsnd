 ...
 *
 * Exercise 3: Folding in playaiff5.
 *
 */
...
int playsample(FILE *fp, bb_result_t result, int volume)
{
    int default_driver;
    int frames_read;
    int count;
    ...
    lseek(fd, result.data.startpos, SEEK_SET);
    ...
    while (toread > 0) {
        ...
        for (volcount = 0; volcount <= frames_read; volcount++) {
            buffer[volcount] /= mypower(2, -volume + 8);
        }
